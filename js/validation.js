...
function handleFormSubmission(formId, submitButtonId) {
    ...
    let submitButton = submitButtonId ? document.getElementById(submitButtonId) : null;

    if (!form) { return; }

    // If the provided submitButtonId is missing or resolves to the form itself,
    // find the actual submit control within the form.
    ...
    form.addEventListener('submit', function() {
        ...
        const isButton = submitButton.tagName === 'BUTTON';
        const originalText = isButton ? submitButton.textContent : submitButton.value;
        ...
        setTimeout(() => {
            ...
            const stored = submitButton.getAttribute('data-original-text');
            if (isButton) {
                submitButton.textContent = stored || 'Submit';
            } else {
                submitButton.value = stored || 'Submit';
            }
        }, 10000);
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    handleFormSubmission('registerForm', 'registerForm');
    handleFormSubmission('loginForm', 'loginForm');
    handleFormSubmission('profileForm', 'profileForm');
    handleFormSubmission('passwordForm', 'passwordForm');
    handleFormSubmission('ideaForm', 'ideaForm');
    handleFormSubmission('commentForm', 'commentForm');
});
