...
// Compute base URL path: project root folder (one level up from /php)
$projectBase = rtrim(dirname(dirname($_SERVER['SCRIPT_NAME'])), '/\\');
if ($projectBase === '') { $projectBase = '/'; }

if ($response['success']) {
    $_SESSION['success_message'] = $response['message'];
    header('Location: ' . rtrim($projectBase, '/') . '/dashboard.php');
} else {
    $_SESSION['error_message'] = $response['message'];
    header('Location: ' . rtrim($projectBase, '/') . '/register.php');
}
exit();
?>